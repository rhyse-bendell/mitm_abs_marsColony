...
class TestInterfacePerAgentDefaults(unittest.TestCase):
    ...
    def test_experiment_ui_defaults_and_agent_count(self):
        ...
        try:
            app.root.withdraw()
            self.assertEqual(app.brain_backend_var.get(), "rule_brain")
            ...
            self.assertEqual(app.local_timeout_var.get(), 240.0)
            ...
            self.assertEqual(app.agent_planner_settings["Engineer"]["planner_timeout_seconds"].get(), 90.0)
            ...
            self.assertFalse(app.enable_startup_llm_sanity_var.get())
            self.assertEqual(app.startup_llm_sanity_timeout_var.get(), 240.0)
            ...
            self.assertFalse(app.unrestricted_local_qwen_mode_var.get())
            self.assertEqual(app.warmup_timeout_var.get(), 180.0)
            self.assertEqual(app.startup_llm_sanity_completion_tokens_var.get(), 2048)
            self.assertEqual(app.planner_completion_tokens_var.get(), 4096)
            self.assertEqual(app.planner_request_policy_var.get(), "cadence_with_dik_integration")
            self.assertEqual(app.planning_interval_steps_var.get(), 60)
        finally:
            app.stop_experiment()
            app.root.destroy()
    ...
    def test_inheritance_and_effective_summary_labels_are_exposed_in_ui(self):
        ...
        try:
            ...
            self.assertIn("Inherited from global Backend: rule_brain", app.agent_inheritance_note_vars[role]["backend"].get())
    ...
    def test_startup_bootstrap_settings_propagate_into_simulation_planner_config(self):
        ...
        try:
            ...
            app.high_latency_local_llm_mode_var.set(True)
            app.unrestricted_local_qwen_mode_var.set(True)
            app.planner_timeout_seconds_var.set(333.0)
            app.warmup_timeout_var.set(222.0)
            app.startup_llm_sanity_completion_tokens_var.set(4096)
            app.planner_completion_tokens_var.set(6144)
            app.high_latency_stale_result_grace_var.set(33.0)
            ...
            self.assertTrue(planner_cfg["high_latency_local_llm_mode"])
            self.assertTrue(planner_cfg["unrestricted_local_qwen_mode"])
            self.assertEqual(planner_cfg["planner_timeout_seconds"], 333.0)
            self.assertEqual(planner_cfg["warmup_timeout_seconds"], 222.0)
            self.assertEqual(planner_cfg["startup_llm_sanity_completion_max_tokens"], 4096)
            self.assertEqual(planner_cfg["planner_completion_max_tokens"], 6144)
            ...
            self.assertEqual(app.sim.planner_defaults.get("planner_completion_max_tokens"), 6144)
            self.assertEqual(app.sim.planner_defaults.get("warmup_timeout_seconds"), 222.0)
            self.assertTrue(app.sim.planner_defaults.get("unrestricted_local_qwen_mode"))
            self.assertEqual(app.sim.planner_defaults.get("high_latency_stale_result_grace_s"), 33.0)
        finally:
            app.stop_experiment()
            app.root.destroy()
