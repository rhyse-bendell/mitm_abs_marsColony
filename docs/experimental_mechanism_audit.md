...
2. Resolution precedence is: mechanism defaults → construct-derived effects → explicit `mechanism_overrides` (with `traits` accepted only as load-time alias).  
...
- Runtime logs now emit `legacy_traits_alias_normalized` when old `traits` payloads are consumed as alias input.  
