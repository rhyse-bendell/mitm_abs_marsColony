...
def aggregate_run_summaries(outputs_root, output_basename="aggregate_run_summaries"):
    ...
    for session_dir in sorted(p for p in outputs_path.iterdir() if p.is_dir()):
        ...
        row = {
            ...
            "configured_brain_backend": metadata.get("configured_brain_backend"),
            "effective_brain_backend": metadata.get("effective_brain_backend"),
            "fallback_backend": metadata.get("fallback_backend"),
            "local_model_name": metadata.get("local_model_name"),
            "local_base_url": metadata.get("local_base_url"),
            "fallback_occurred": metadata.get("fallback_occurred"),
